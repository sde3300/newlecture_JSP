...
public class Calc2 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		ServletContext application =request.getServletContext();						//값을 저장해야하는데 어디다 저장해야하는가? request에보면 ServletContext()가있다. 이것이 어플리케이션 저장소이다 application이라는 변수에다가 getservletContext()를 저장하자
		...
		//계산
		if(op.equals("=")) {															//왜 if문에 =(equal)로했냐면 덧셈과 뺄셈은 하는일이 같다(저장). 하지만 =는 계산하는것이다. 
			int x = (Integer)application.getAttribute("value");							//이전에 저장한 값을 어플리케이션 저장소에서 꺼내와야하는데 application.getAttribute("담을떄사용했던 값")메소드를 사용한다. 그리고 (Integer)을 안했을 떄 빨간줄이 뜨는것은 Object로 값을 반환하기 때문이다.
			int y = v;																	//지금 사용자가 저장한 값
		...
		// 값을 저장
			...
			application.setAttribute("op",op);  						 				//오퍼레이터도 같은 방식으로 저장해주기위해서 key와 value를 저장해주자 이것들을 두고두고 쓸수있다.																						
																						//전달된내용이 op이 뭐냐인가에 따라서 저장해야할수도있고 계산이 될 수도 있다.	
		}		
	}
}